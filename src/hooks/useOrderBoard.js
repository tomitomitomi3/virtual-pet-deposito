...
const VALID_TRANSITIONS = {
  ...
  en_preparacion: ['pendiente', 'preparado', 'cancelado'],
  preparado: ['en_preparacion', 'despachado', 'cancelado'],
  despachado: ['preparado', 'entregado', 'cancelado'],
...
export const useOrderBoard = () => {
          ...
          if (data.type === 'order_created' || data.type === 'order_updated') {
            ...
            setOrders(prev => {
              ...
              if (index !== -1) {
                ...
                // Marcamos como reenvío si vuelve de despachado a preparado
                const isReturning = data.order.estado === 'preparado' && existingOrder.estado === 'despachado';
                
                newOrders[index] = {
                  ...data.order,
                  isReenvio: existingOrder.isReenvio || isReturning
                };
                return newOrders;
              }
  ...
  const handleMove = async (result) => {
    ...
    setOrders(prev => {
      ...
      // Lógica de reenvío por falla en entrega
      const isReturningFromDelivery = movedItem.estado === 'despachado' && destination.droppableId === 'preparado';
      
      const updatedItem = { 
        ...movedItem, 
        estado: destination.droppableId,
        isReenvio: movedItem.isReenvio || isReturningFromDelivery
      };
    ...
    if (destination.droppableId !== source.droppableId) {
      try {
        ...
        setOrders(prevOrders); // Rollback si falla la API
      }
    }
  };
